...
! Added for Flare triggering simulations
  ...
  REAL(num), DIMENSION(:,:,:), ALLOCATABLE :: ctv, ctx, cty, ctz
  REAL(num), DIMENSION(:,:,:), ALLOCATABLE :: ellv, ellx, elly, ellz
  REAL(num) :: rotamp, phic, xx, yy, zz, cr, cz
...
! Flare triggering parameters
  ...
  REAL(num) :: x_amp = 1.0_num
  REAL(num) :: y_amp = 0.2_num
