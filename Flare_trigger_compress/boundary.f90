...
CONTAINS
  ...
  ! Define driving parallel and perpendicular to the PIL

  double precision :: drivx,drivy
  drivx = x_amp*sin(2.0_num*pi*omega*time)
  drivy = y_amp*sin(2.0_num*pi*omega*time)
  ...
  ! Velocity aligned injected field coordinates
    ...
    DO ix = -2,nx+2
       DO iy = -2,ny+2
          DO iz = -2, 0
            xx = (xb(ix)-drivx)*cos(phic) + (yb(iy)-drivy)*sin(phic)
            yy =-(xb(ix)-drivx)*sin(phic) + (yb(iy)-drivy)*cos(phic)
    ...
    DO ix = -2,nx+2
       DO iy = -1,ny+2
          DO iz = -1, 0
            xx = (xb(ix)-drivx)*cos(phic) + (yc(iy)-drivy)*sin(phic)
            yy =-(xb(ix)-drivx)*sin(phic) + (yc(iy)-drivy)*cos(phic)
    ...
    DO ix = -1,nx+2
       DO iy = -2,ny+2
          DO iz = -1, 0
            xx = (xc(ix)-drivx)*cos(phic) + (yb(iy)-drivy)*sin(phic)
            yy =-(xc(ix)-drivx)*sin(phic) + (yb(iy)-drivy)*cos(phic)
            zz = zc(iz)-(z0 + vzf*min(t1-t0,time-t0))
            cr = sqrt(yy**2.0+zz**2.0)
            cz = xx
    ...
    ! Bz aligned injected field coordiantes

    DO ix = -1,nx+2
       DO iy = -1,ny+2
          DO iz = -2, 0
            xx = (xc(ix)-drivx)*cos(phic) + (yc(iy)-drivy)*sin(phic)
            yy =-(xc(ix)-drivx)*sin(phic) + (yc(iy)-drivy)*cos(phic)
            zz = zb(iz)-(z0 + vzf*min(t1-t0,time-t0))
            cr = sqrt(yy**2.0+zz**2.0)
            cz = xx
  ...
  SUBROUTINE density_bcs
    ...
    ! We also fix the density one cell into the domain at the lower boundary.
